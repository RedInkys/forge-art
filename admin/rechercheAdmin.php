...
if(isset($_GET['rechercheAdmin']) && !empty($_GET['rechercheAdmin']))
{
  $toutLesMembres = $pdo->query("SELECT * FROM membre");
  $recherche = htmlspecialchars($_GET['rechercheAdmin']);
  $toutLesMembres = $pdo->query('SELECT * FROM membre WHERE pseudo LIKE "%'.$recherche.'%"');
} 
...
<?php if($toutLesMembres->rowcount() > 0): ?>
    <?php while($membres = $toutLesMembres->fetchAll(PDO::FETCH_ASSOC)): ?>
            ...
            <div class="mt-8 flow-root">
                ...
                <td>
                    <div class="ml-4 gap-2 flex flex-col">
                        ...
                        <div class="font-medium text-white bg-red-700 rounded-md p-1"><a href="?action=suppression&id=<?= $membre['id_membre'] ?>" onclick="return confirm('Etes vous sur de vouloir supprimer le compte ?')">Suppr.</a></div>
                    </div>
                </td>
...
<?php else: ?>
    ...
    <a href="<?= URL ?>admin/allInfo.php" class="p-2 bg-[#C8B6FF] rounded-md text-white hover:bg-[#FFD6FF]">Retour</a>
    </div>
    <?php endif; ?>
